...
class PatternAnalyzer(object):
    ...
    def __init__(self):
        ...
        for key, pats in self.all_pats.items():
            ...
            kv_contain_short, kvpats_short, kvdicts_short = self._make_vpats(contain_vpats, vpats, dict_pats, long=False)
        ...
        for key, value in self.parsed_pats.items():
            ...
            self.parsed_pats[key] = (self._rank_group(kvpats = long[1], kvdicts=long[2],kv_contain=long[0]), self._rank_group(kvpats = short[1], kvdicts=short[2],kv_contain=short[0]))
    ...
    def _parse_from_file(self, pat_fname):
        ...
        vpats = [pat for pat in pats if pat.startswith('$') and 'str' not in pat and 'contain' not in pat]  #正则表达式非contain的
