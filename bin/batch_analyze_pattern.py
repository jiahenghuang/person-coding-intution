...
if __name__ == '__main__':
    ...
    import pandas as pd
    data = pd.read_csv('data/testdata/A话术是否单身问题测试数据.csv')
    ...
    for text in texts:
        ...
        if textLen > 6:
            ...
            flag = False
        print(text)
        asr_text = asr_analyzer.analyze(text, long=flag)
        result = pattern_analyzer.parse(asr_text, long=flag, keywordVersion='V20190402')
        if result:
            group = result.group
            pattern = result.word
        else:
    ...
    data['isasr'] = isasr
    data.to_csv('data/testdata/A话术上线前测试.csv', index = 0)
